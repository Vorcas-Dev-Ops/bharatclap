...
// @route   GET /api/payments/analytics/revenue
// @access  Private/Admin
export const getRevenueAnalytics = async (req: Request, res: Response): Promise<void> => {
  try {
    const allCompletedPayments = await Payment.find({ payment_status: 'completed' }).lean();
    
    const totalRevenue = allCompletedPayments.reduce((acc, curr) => acc + curr.amount, 0);
    ...
    allCompletedPayments.forEach(payment => {
      const pDate = new Date(payment.payment_date || payment.createdAt);
      if (pDate >= firstDayThisMonth) {
        thisMonthRevenue += payment.amount;
        transactionsThisMonth++;
      } else if (pDate >= firstDayLastMonth && pDate < firstDayThisMonth) {
        lastMonthRevenue += payment.amount;
      }
    });
    
    let growth = 0;
    if (lastMonthRevenue === 0 && thisMonthRevenue > 0) growth = 100;
    else if (lastMonthRevenue > 0) growth = ((thisMonthRevenue - lastMonthRevenue) / lastMonthRevenue) * 100;
