...
import { X, Wrench, Save, Tag, Activity, Palette, DollarSign, List } from 'lucide-react';
...
const AccessoryModal: React.FC<AccessoryModalProps> = ({ isOpen, onClose, accessory, categories, onSave }) => {
    ...
    if (isOpen) {
      ...
      if (accessory) {
        setFormData({
          ...
          category: accessory.category?._id || accessory.category || '',
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-hidden flex flex-col">
                ...
                <div className="space-y-4 relative z-10">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 flex items-center gap-2">
                      <Tag size={12} className="text-blue-500" /> Title
                    </label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 flex items-center gap-2">
                        <List size={12} className="text-blue-500" /> Category
                      </label>
                      <select
                        required
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 flex items-center gap-2">
                        <Palette size={12} className="text-blue-500" /> Image URL
                      </label>
                      <input
                        type="url"
                        required
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black text-gray-400 tracking-widest ml-1 flex items-center gap-2">
                      <Wrench size={12} className="text-blue-500" /> Description
                    </label>
                    <textarea
                      required
            ...
            <div className="flex-shrink-0 px-8 pb-8 pt-4 bg-white border-t border-gray-50 flex gap-3">
              ...
              >
                <Save size={14} /> {accessory ? 'Update Accessory' : 'Save Accessory'}
              </button>
            </div>
          </form>
        </motion.div>
