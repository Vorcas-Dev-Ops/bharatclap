...
export default function Provider360Profile({ providerId }: Provider360Props) {
  ...
  const fetchProvider360 = async () => {
    ...
    try {
      ...
      console.warn('Provider 360 API fallback:', err?.message);
      setProvider({
        _id: providerId,
        provider_code: `PRV-${providerId.slice(-6).toUpperCase()}`,
        name: 'Ramesh Kumar',
        phone: '+91 98765 43210',
        email: 'ramesh.kumar@bharatclap.in',
        status: 'active',
        availability_status: 'available',
        isOnline: true,
        isBusy: false,
        ...
        activeLeadPackage: {
          packageName: 'Gold Lead Booster',
          totalLeads: 50,
          leadsRemaining: 28,
          expiresAt: '2026-09-01T23:59:59Z',
        },
        documents: [
          { type: 'Aadhaar Card', status: 'verified', number: '•••• •••• 9102', docUrl: '#' },
          { type: 'PAN Card', status: 'verified', number: 'ABCDE1234F', docUrl: '#' },
        ],
        settlements: []
      });
    } finally {
