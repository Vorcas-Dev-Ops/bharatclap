...
import {
  Gift, Copy, CheckCheck, Users, Clock,
      ...
      <div className="max-w-3xl mx-auto px-4 py-10 space-y-8">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex-1 bg-slate-100 rounded-2xl px-4 py-3.5 font-black text-sm text-slate-700 tracking-[0.15em] border-2 border-dashed border-slate-300">{referralCode}</div>
            <button onClick={handleCopy}
              className={`flex items-center gap-2 px-5 py-3.5 rounded-2xl text-sm font-black transition-all ${codeCopied ? "bg-emerald-500 text-white" : "bg-[#1D2B83] text-white hover:opacity-90"}`}>
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {shareOptions.map(s => (
                <button key={s.label} className={`${s.color} text-white rounded-2xl py-3 flex flex-col items-center gap-1.5 transition-all hover:scale-105 text-xs font-black`}>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-50"><h2 className="text-base font-black text-slate-900">How It Works</h2></div>
            ...
            {[
              ...
              <React.Fragment key={s.step}>
                <div className="flex-1 flex flex-col items-center text-center gap-2">
                  <div className="w-10 h-10 rounded-full bg-[#1D2B83] text-white font-black text-sm flex items-center justify-center shadow-lg shadow-blue-900/20">{s.step}</div>
                  <p className="text-sm font-black text-slate-800">{s.label}</p>
                  <p className="text-[11px] text-slate-400 font-medium">{s.desc}</p>
                </div>
                {i < arr.length - 1 && <div className="hidden sm:flex items-center text-slate-300"><ArrowRight className="w-5 h-5" /></div>}
              </React.Fragment>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
            <h2 className="text-base font-black text-slate-900 flex items-center gap-2"><Users className="w-4 h-4 text-[#1D2B83]" /> Referral History</h2>
          </div>
          <div className="divide-y divide-slate-50">
            {referralHistory.map((r, i) => (
              <div key={i} className="px-6 py-4 flex items-center gap-4 hover:bg-slate-50/50 transition-colors">
                <div className="w-9 h-9 rounded-xl bg-blue-100 flex items-center justify-center flex-shrink-0"><UserPlus className="w-4 h-4 text-[#1D2B83]" /></div>
                <div className="flex-1">
                  <p className="text-sm font-black text-slate-800">{r.name} joined</p>
                  <p className="text-[11px] text-slate-400 font-medium flex items-center gap-1 mt-0.5"><Clock className="w-3 h-3" /> {r.date}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black text-emerald-600">+₹{r.reward}</p>
                  <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${r.status === "earned" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}>
