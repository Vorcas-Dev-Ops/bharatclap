...
const LocationModal: React.FC<LocationModalProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  const handleDeleteClick = async (addressId: string) => {
    ...
    try {
      ...
      messageApi.error(err.response?.data?.message || "Failed to remove address");
    }
  };
                ...
                <div className={activeTab === "addresses" ? "block space-y-6" : "hidden"}>
                  <div className="grid grid-cols-1 gap-3">
                    <button
                      ...
                      disabled={isLocating}
                      className="w-full flex items-center gap-4 p-4 rounded-2xl bg-[#1D2B83] text-white hover:bg-[#1D2B83]/90 shadow-lg shadow-blue-900/20 transition-all group"
                    >
                      <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                        {isLocating ? <Loader2 className="w-5 h-5 animate-spin" /> : <Navigation className="w-5 h-5" />}
                      </div>
                      <div className="flex-1 text-left">
                        <p className="font-bold text-sm">Use my current location</p>
                        <p className="text-[10px] opacity-70 font-bold uppercase tracking-wide">Detect live GPS coordinates</p>
                      </div>
                  ...
                  <div className="space-y-3">
                    <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 px-1">Saved Addresses</h3>
                    {loadingAddresses ? (
                      ...
                      <div className="space-y-3">
                        {addresses.map((addr) => (
                          ...
                          >
                            ...
                            >
                              <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors ${addr.address_label === 'Home' ? 'bg-violet-50 text-violet-600' : addr.address_label === 'Office' ? 'bg-sky-50 text-sky-600' : 'bg-amber-50 text-amber-600'}`}>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <p className="font-bold text-slate-800 text-sm truncate">{addr.address_label || addr.city}</p>
                                  {addr.is_default && <span className="text-[8px] font-black uppercase tracking-tighter bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded flex-shrink-0">Default</span>}
                                </div>
                                <p className="text-xs text-slate-500 font-medium line-clamp-1 mt-0.5">{addr.house_name || addr.address_line}{(addr as any).building_name ? `, ${(addr as any).building_name}` : ''}</p>
                                <p className="text-[10px] text-slate-400 font-medium mt-0.5">{[addr.area, addr.city, addr.pincode].filter(Boolean).join(', ')}</p>
                              </div>
                            </div>
