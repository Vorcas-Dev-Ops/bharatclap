...
export default function NotificationsPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <Navbar />
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 py-8 mt-8 space-y-8">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-800 tracking-tight">Notifications</h1>
            <p className="text-slate-500 font-medium">Keep track of your booking confirmations, payments, and discounts.</p>
          </div>
          <div className="flex items-center gap-3">
            <button 
              onClick={markAllAsRead}
              className="text-sm font-bold text-[#1D2B83] hover:underline px-4 py-2 hover:bg-[#1D2B83]/5 rounded-xl transition-all"
            >
              Mark all as read
            </button>
          </div>
                    ...
                    <div className="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {!notif.is_read && (
                        <button 
                          ...
                          className="p-2 bg-[#1D2B83]/10 text-[#1D2B83] rounded-lg hover:bg-[#1D2B83]/20 transition-all"
                        >
