...
export interface IWalletAuditLog extends Document {
  ...
  providerName: string;
  action: 'Wallet Credit' | 'Wallet Debit' | 'Freeze Wallet' | 'Unfreeze Wallet' | 'Manual Adjustment';
  amount: number;
...
const walletAuditLogSchema = new Schema<IWalletAuditLog>(
  {
    ...
    action: {
      type: String,
      enum: ['Wallet Credit', 'Wallet Debit', 'Freeze Wallet', 'Unfreeze Wallet', 'Manual Adjustment'],
      required: true
    },
