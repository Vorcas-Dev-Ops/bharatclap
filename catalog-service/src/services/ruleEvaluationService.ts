import { ITimeSlotRule } from '../models/TimeSlotRule';

export interface EvaluationContext {
  slotTime: string; // e.g. "18:00" or "06:00 PM" or "18:00-19:00"
  basePrice?: number;
  categoryId?: string;
  subserviceId?: string;
  city?: string;
...
export interface EvaluatedRuleResult {
  appliedRules: {
    ...
    chargeDelta: number; // calculated monetary impact (+ or -)
    reason: string;
  }[];
  ignoredRules: {
    ruleId: string;
  ...
  netSlotSurcharge: number; // totalExtraCharge - totalDiscount
  isPeak: boolean;
}
...
function isSlotInRuleWindow(slotTime: string, startTime: string, endTime: string): boolean {
...
export function evaluateTimeSlotRules(
  allRules: ITimeSlotRule[],
  ...
  const appliedRules: EvaluatedRuleResult['appliedRules'] = [];
  const ignoredRules: EvaluatedRuleResult['ignoredRules'] = [];
